...
The project uses **3-Tier Architecture** to enhance maintainability and scalability. The directories are organized according to these layers. Below is an overview of the key directories and files:
...
4. **Database Layer**: Interacts with the database to perform operations, takes or returns Data objects (such as `User` or `Session` defined in `model/`).
